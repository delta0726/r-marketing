...
# - 探索的因子分析は共通因子の抽出を目的としている（データの構造が似た成分を抽出）
#   --- いずれも多くの変数を少ない因子(成分)で説明しようとしている
#   --- 因子分析では説明する因子数を予め決定する（主成分分析は計算時には成分数の指定はしない）
...
# - 固有値の大きい特徴量の性質を強く持つ傾向にある（固有値が1を超える特徴量の数などで因子数を指定しているため）
#   --- 因子負荷量を解釈することで意味づけを行う
...
# - 回転は各因子間に相関についての仮定の置き方によって決める
#   --- 因子間の相関なのでデータから自然に決まるものではなく指定することが可能
...
# ＜ポイント＞
# - 因子分析では潜在変数と各属性の関係を解釈することができる（SEMプロット）
#   --- 因子負荷量のヒートマップでは関係性は表現できない
